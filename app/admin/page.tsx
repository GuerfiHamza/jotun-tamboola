...
function StatusBadge({ s }: { s: ParticipantStatus | InvoiceStatus }) {
...
function InvoiceCard({
  ...
  async function retryAnalysis() {
    ...
    } catch { setRetryMsg('Erreur réseau.'); }
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center gap-2">
        <span className="font-medium text-white/70 truncate max-w-32">{inv.original_name}</span>
        ...
        {editing ? (
          <div className="flex items-center gap-1">
            <input
              ...
              autoFocus
              className="rounded px-1.5 py-0.5 text-xs w-20 text-white outline-none"
              style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(239,68,68,0.4)' }}
            />
            <span className="text-white/30">DA</span>
            ...
            <button onClick={() => { setEditing(false); setAmountInput(inv.amount_detected ?? ''); setError(''); }}
              className="rounded px-1.5 py-0.5 text-xs text-white/50 hover:text-white transition-colors"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setEditing(true)}
              className="text-white/25 hover:text-amber-400 transition-colors" title="Modifier">
      ...
      <div className="text-white/25">{new Date(inv.uploaded_at).toLocaleString('fr-DZ')}</div>
...
function StatCard({ label, value, color, icon }: { label: string; value: number | string; color: string; icon: React.ReactNode }) {
  return (
    ...
    >
      ...
      <div className="text-xs font-medium mt-1 uppercase tracking-wide" style={{ color: 'rgba(255,255,255,0.35)' }}>{label}</div>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    } catch { setFetchError('Impossible de charger les statistiques.'); }
  }, [router]);
  ...
  const fetchParticipants = useCallback(async (p = 1) => {
    ...
    try {
      ...
      setPages(data.pages);
      setPage(p);
    } catch { setFetchError('Erreur de chargement de la liste. Réessayez.'); }
  ...
  async function openParticipant(id: number) {
    ...
    } catch { setFetchError('Impossible de charger ce participant.'); }
  }
  ...
  const statCards = stats ? [
    { label: 'Total inscrits',    value: stats.participants.total,             color: '#ef4444', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z"/></svg> },
    { label: 'Peintres',          value: stats.participants.painters,          color: '#f59e0b', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg> },
    { label: 'Approuvés',         value: stats.participants.approved,          color: '#10b981', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg> },
    { label: 'En attente',        value: stats.participants.pending,           color: '#fbbf24', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd"/></svg> },
    { label: 'Factures soumises', value: stats.invoices.total,                 color: '#3b82f6', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"/></svg> },
    { label: 'À vérifier',        value: stats.invoices.needs_attention ?? 0,  color: '#f97316', icon: <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"/></svg> },
  ] : [];

  return (
    <main className="min-h-screen" style={{ background: '#08080f', color: '#f9fafb' }}>

      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header
        className="sticky top-0 z-40 px-6 py-3.5 flex items-center gap-3"
        style={{ background: 'rgba(8,8,15,0.92)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 4px 32px rgba(0,0,0,0.4)' }}
      >
        {/* Brand */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        >
          <div className="relative flex-1 min-w-48">
            <svg viewBox="0 0 20 20" className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: 'rgba(255,255,255,0.25)' }} fill="currentColor">
          ...
          >
            ...
            <option value="pending"  style={{ background: '#0f0f1c' }}>En attente</option>
            <option value="approved" style={{ background: '#0f0f1c' }}>Approuvés</option>
            <option value="rejected" style={{ background: '#0f0f1c' }}>Refusés</option>
          </select>
          ...
          <span className="text-xs ml-auto" style={{ color: 'rgba(255,255,255,0.3)' }}>{total} résultat(s)</span>
        </div>
        ...
        <div className="flex gap-5 items-start">
          ...
          {/* Table */}
          <div className="flex-1 min-w-0 rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                    {['Nom', 'Téléphone', 'Wilaya', 'Peintre', 'Statut', 'Factures', 'Date', 'Actions'].map(h => (
                      <th key={h} className="text-left px-4 py-3.5 text-xs font-bold uppercase tracking-wide" style={{ color: 'rgba(255,255,255,0.4)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {loadingList ? (
                    <tr><td colSpan={8} className="text-center py-16" style={{ color: 'rgba(255,255,255,0.2)' }}>
                    ...
                    <tr><td colSpan={8} className="text-center py-16" style={{ color: 'rgba(255,255,255,0.2)' }}>Aucun résultat</td></tr>
                  ) : participants.map(p => (
                    <tr
                      key={p.id}
                      ...
                      onMouseEnter={e => { if (selected?.id !== p.id) (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.03)'; }}
                    ...
                    >
                      <td className="px-4 py-3.5 font-medium text-white">
                        ...
                        {(p.needs_attention ?? 0) > 0 && (
                          <span
                            ...
                            title="Facture non validée automatiquement"
                          >
                            Vérifier
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{p.phone}</td>
                      <td className="px-4 py-3.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{p.wilaya}</td>
                      <td className="px-4 py-3.5 text-center">
                        {p.is_painter ? (
                          <span className="inline-flex items-center justify-center w-5 h-5 rounded-md" style={{ background: 'rgba(245,158,11,0.15)', color: '#fbbf24' }}>
                            <svg viewBox="0 0 16 16" className="w-3 h-3" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3.5">
                        <span className="font-bold tabular-nums" style={{ color: 'rgba(255,255,255,0.7)' }}>{p.invoice_count}</span>
                        {p.best_invoice != null && (
                          <span className={`ml-2 text-xs font-semibold ${p.best_invoice >= 20000 ? 'text-emerald-400' : 'text-red-400'}`}>
                            {Number(p.best_invoice).toLocaleString('fr-DZ')} DA
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3.5 text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>{new Date(p.created_at).toLocaleDateString('fr-DZ')}</td>
                      <td className="px-4 py-3.5">
            ...
            {pages > 1 && (
              ...
              >
                ...
                <span className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>Page {page} / {pages}</span>
                <button
          ...
          {selected && (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-bold text-white">Détails du participant</h3>
                <button
              ...
              {/* Info rows */}
              <div className="space-y-2.5 text-sm mb-5 pb-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {[
                  { label: 'Nom',       value: selected.full_name },
                  { label: 'Téléphone', value: selected.phone     },
                  { label: 'Wilaya',    value: selected.wilaya    },
                  { label: 'Peintre',   value: selected.is_painter ? 'Oui' : 'Non' },
                  { label: 'Inscrit le',value: new Date(selected.created_at).toLocaleString('fr-DZ') },
                ].map(row => (
                  <div key={row.label} className="flex justify-between items-start gap-3">
                    <span style={{ color: 'rgba(255,255,255,0.35)' }}>{row.label}</span>
                    <span className="font-medium text-white text-right text-xs">{row.value}</span>
                  </div>
                ))}
              ...
              {/* Invoices */}
              <div>
                <h4 className="text-[10px] font-bold uppercase tracking-widest mb-3" style={{ color: 'rgba(255,255,255,0.3)' }}>
                  Factures ({invoices.length})
                </h4>
                {invoices.length === 0 ? (
                  <p className="text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>Aucune facture soumise</p>
                ) : (
